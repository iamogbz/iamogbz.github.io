...
function encode(s) {
    ...
    return Object.keys(positions).sort().map(c => {
        return `${c}${DELIM_CPOS}${positions[c].sort().join(DELIM_POS)}`;
    }).join(DELIM_GROUP)
}
...
export function decode(s) {
    ...
    return Object.keys(positions).sort((a, b) => Number(a) - Number(b)).map(i => positions[i]).join(EMPTY_STRING);
}
